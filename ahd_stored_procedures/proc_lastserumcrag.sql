-- PROCEDURE: ahd.proc_lastserumcrag(character varying)

-- DROP PROCEDURE IF EXISTS ahd.proc_lastserumcrag(character varying);

CREATE OR REPLACE PROCEDURE ahd.proc_lastserumcrag(
	IN datim_id character varying)
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE start_time TIMESTAMP;
DECLARE end_time TIMESTAMP;
DECLARE ctelastserumcrag_partition text;
...
ctelastserumcrag_partition := CONCAT('cte_lastserumcrag_',datim_id);
...
EXECUTE FORMAT('TRUNCATE ahd.%I',ctelastserumcrag_partition);

RAISE NOTICE 'successfully truncate % table', ctelastserumcrag_partition;

EXECUTE FORMAT('INSERT INTO ahd.%I
SELECT personuuid12 lastserumcrag_personuuid,date_result_reported AS dateOflastserumcrag, 
result_reported AS lastserumcrag, ods_datim_id lastserumcrag_partition_datim_id
...
WHERE lt.lab_test_id = 69 AND lr.date_result_reported <= %L) dt
WHERE dt.rowNum = 1',
ctelastserumcrag_partition,laboratorytest_partition,
...
VALUES (%L,%L, %L, %L)',
ahdmonitoringpartition,ctelastserumcrag_partition,start_time,end_time,datim_id);

END
$BODY$;
ALTER PROCEDURE ahd.proc_lastserumcrag(character varying)
    OWNER TO lamisplus_etl;


-- CALL ahd.proc_lastserumcrag('A1xxdELs2fm');
