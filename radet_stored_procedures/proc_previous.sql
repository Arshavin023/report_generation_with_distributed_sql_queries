-- PROCEDURE: expanded_radet_client.proc_previous(character varying)

-- DROP PROCEDURE IF EXISTS expanded_radet_client.proc_previous(character varying);

CREATE OR REPLACE PROCEDURE expanded_radet_client.proc_previous(
	IN datim_id character varying)
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE start_time TIMESTAMP;
DECLARE end_time TIMESTAMP;
DECLARE cteprevious_partition text;
...
DECLARE previous_quarter_date DATE;
DECLARE radetmonitoringpartition text;

BEGIN

SELECT cast(to_char(date_trunc('quarter', date (cast((
select date from expanded_radet.period 
where is_active) as varchar)))::date - 1,'yyyy-MM-DD') as date) as date ----?4
INTO previous_quarter_date;

cteprevious_partition := CONCAT('cte_previous_',datim_id);
...
EXECUTE FORMAT('TRUNCATE expanded_radet_client.%I',cteprevious_partition);

RAISE NOTICE 'successfully truncate % table', cteprevious_partition;

EXECUTE FORMAT('INSERT INTO expanded_radet_client.%I
SELECT DISTINCT ON (pharmacy.person_uuid) pharmacy.person_uuid AS prePersonUuid,
pharmacy.ods_datim_id pre_ods_datim_id,
...
cteprevious_partition,previous_quarter_date,previous_quarter_date,hivartpharmacy_partition,
hivartpharmacy_partition, hivartpharmacyregimens_partition,hivenrollment_partition,
hivregimen_partition,hivregimentype_partition,previous_quarter_date,previous_quarter_date,
hivstatustracker_partition,previous_quarter_date,hivenrollment_partition,
...
VALUES (%L,%L, %L, %L)',
radetmonitoringpartition,cteprevious_partition,start_time,end_time,datim_id);

END
$BODY$;
ALTER PROCEDURE expanded_radet_client.proc_previous(character varying)
    OWNER TO lamisplus_etl;
CALL expanded_radet_client.proc_previous('tZy8wIM53xT');