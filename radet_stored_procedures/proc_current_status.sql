-- PROCEDURE: expanded_radet_client.proc_current_status(character varying)

-- DROP PROCEDURE IF EXISTS expanded_radet_client.proc_current_status(character varying);

CREATE OR REPLACE PROCEDURE expanded_radet_client.proc_current_status(
...
-- EXECUTE FORMAT('TRUNCATE expanded_radet_client.%I',ctecurrentstatus_partition);

DELETE FROM expanded_radet_client.cte_current_status
...
VALUES (%L,%L, %L, %L)',
radetmonitoringpartition,ctecurrentstatus_partition,start_time,end_time,datim_id);

END
$BODY$;
ALTER PROCEDURE expanded_radet_client.proc_current_status(character varying)
    OWNER TO lamisplus_etl;

CALL expanded_radet_client.proc_current_status('tZy8wIM53xT');